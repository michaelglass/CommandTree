...
let overrides =
    Map.ofList
        [ // Tree.fs: F# compiler-generated branch for helpWithGlobals Leaf fallback
          "Tree.fs", (96.0, 93.0)
          // Reflection.fs: Compiler-generated branches for type matching, prefix matching, duplicated env parse logic
          "Reflection.fs", (93.0, 81.0)
...
let parseCoverageReport (xmlPath: string) : FileCoverage list =
    ...
    |> List.map (fun (fileName, items) ->
        ...
        let branchMap = Collections.Generic.Dictionary<int, int * int>()
...
let printResults (result: CoverageResult) =
    let printFile prefix (r: FileResult) =
        ...
        printfn "%s %s: line=%.1f%%%s" prefix shortName r.File.LinePct branchInfo
