...
let fsproj = "src/CommandTree/CommandTree.fsproj"
let dllPath = "src/CommandTree/bin/Release/net10.0/CommandTree.dll"
let repoUrl = "https://github.com/michaelglass/CommandTree"
...
module Api =
    ...
    let extractFromTag (tag: string) : ApiSignature list =
        ...
        let tempDir = Path.Combine(Path.GetTempPath(), sprintf "commandtree-api-check-%s" (Guid.NewGuid().ToString("N").[..7]))
        ...
        try
            ...
            Shell.runOrFail "dotnet" (sprintf "build %s/src/CommandTree/CommandTree.fsproj -c Release --verbosity quiet" tempDir) |> ignore

            let tempDllPath = Path.Combine(tempDir, "src/CommandTree/bin/Release/net10.0/CommandTree.dll")
